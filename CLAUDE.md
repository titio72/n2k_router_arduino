...
| `P` | Change BLE pairing passkey (6 digits, 100000-999999) |

Pairing: every device ships with the same factory-default passkey, `BLE_PASSKEY_FACTORY_DEFAULT` (666666) in `src/Conf.h` - Victron-style, the app should prompt the user to change it (`P` command) the first time they connect. The passkey is persisted in NVS (`Configuration::save_ble_passkey()`/`get_ble_passkey()`); `Configuration::is_ble_passkey_default()` tells the app whether it's still the factory value, surfaced as a flag byte in the BLE data blob (`BUFFER_OFFSET_BLE_PASSKEY_DEFAULT`) so the UI can prompt for a change. A `P` command takes effect immediately, without a reboot (`BLEConf::loop()` pushes passkey changes to the BLE stack via `BTInterface::change_passkey()`); already-paired phones are unaffected since the passkey only matters during initial pairing. 0 means no passkey (BLE writes open, native/test builds only). Writing to the `conf`/`command` characteristics requires an encrypted, MITM-authenticated (passkey) link; reads stay open. Heartbeats go to a separate `heartbeat` characteristic (`BLE_HEARTBEAT_UUID`) that is writable without pairing and only resets the inactivity timer (`h` on `command` still works for paired clients). Any number of clients can pair (`CONFIG_BT_NIMBLE_MAX_BONDS=10`).
